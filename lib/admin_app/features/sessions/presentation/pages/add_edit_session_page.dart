...
class _AddEditSessionPageState extends State<AddEditSessionPage> {
  ...
  void initState() {
    ...
    if (isEditMode) {
      ...
      _existingGalleryUrls = List.from(session.galleryImageUrls);
    }
  }
  ...
  Future<void> _selectDate(BuildContext context) async {
    final picked = await showDatePicker(
      context: context,
      initialDate: isEditMode ? widget.session!.date : DateTime.now(),
    ...
    if (_formKey.currentState!.validate()) {
      if (isEditMode) {
        final updated = SessionEntity(
          ...
          coverImageUrl: widget.session!.coverImageUrl,
          galleryImageUrls: widget.session!.galleryImageUrls,
        );
        context.read<SessionBloc>().add(
          UpdateSessionButtonPressed(
            session: updated,
